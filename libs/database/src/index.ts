...
import { bundle as queueBundle, tableData as queueTableData, fakerData as queueFakerData } from './lib/queue.js';
import { bundle as shopBundle, tableData as shopTableData , fakerData as shopFakerData } from './lib/shops.js';
import { bundle as userBundle, tableData as userTableData, fakerData as userFakerData } from './lib/users.js';
import { bundle as parkingBundle, tableData as parkingTableData, fakerData as parkingFakerData } from './lib/parking.js'
import { bundle as parkingSpotBundle, tableData as parkingSpotTableData, fakerData as parkingSpotFakerData } from './lib/parking_spot.js'
import { bundle as featureBundle, tableData as featureTableData, fakerData as featureFakerData } from './lib/features.js'
...
type TableName = 'features' | 'parking_spot' | 'parking' | 'shop_queue' | 'shops' | 'users';
const tables = {
    users: userTableData,
...
const fakeInserter = {
    ...
    parking_spot: () => bundles.parking_spot.addParkingSpot(parkingSpotFakerData()),
...
async function ensureTable(table_name: TableName, data: string[]) {
    ...
    await pool.query(
        `CREATE TABLE ${table_name} (${data.join(", ")});`
    );
    for (let _ = 0; _ < 100; _++) {
        try {
...
export async function ensureTables() {
    try {
        for (const table_name of Object.keys(tables)) {
            await ensureTable(table_name as TableName, tables[table_name as TableName]);
        }
    } catch (err) {
