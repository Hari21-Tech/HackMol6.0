...
const CREATE_QUERY = 'INSERT INTO parking (name, number_of_empty_spots, maximum_number_of_spots) VALUES ($1, $2, $3);';
const GET_QUERY = 'SELECT * FROM parking WHERE id=$1;';
const DELETE_QUERY = 'DELETE FROM parking WHERE id=$1;';
const UPDATE_QUERY = (name: string | null, number_of_empty_spots: number | null, maximum_number_of_spots: number | null): { query: string, values: string[] } => {
    const query = [];
    const values = [];
    let paramIndex = 1;
    if (name !== null) {
...
function createParking(
    ...
    return Result(async () => {
        return await pool.query<ParkingEntry>(CREATE_QUERY, [name, number_of_empty_spots, maximum_number_of_spots]);
    });
}
...
function updateParking(
    pool: Pool,
    parking_id: number,
    { name, number_of_empty_spots, maximum_number_of_spots }: Nullable<ParkingData>,
): DatabaseReturn<ParkingEntry> {
    return Result(async () => {
        const { query, values } = UPDATE_QUERY(name, number_of_empty_spots, maximum_number_of_spots);
