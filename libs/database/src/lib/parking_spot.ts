...
export const fakerData: () => ParkingSpotData = () => ({
    ...
    spot: alphaChars[Math.round(Math.random() * 26)].toUpperCase() + Math.round(Math.random() * 69).toString()
})

const CREATE_QUERY = 'INSERT INTO parking_spot (parking_id, floor, spot) VALUES ($1, $2, $3) ON CONFLICT (parking_id, position_label) DO NOTHING;';
...
function addParkingSpot(
    ...
    return Result(async () => {
        return await pool.query<ParkingSpotEntry>(CREATE_QUERY, [parking_id, floor, spot]);
    });
}
