...
import { randUserName, randLatitude, randLongitude, randProductDescription, randUrl, randNumber } from '@ngneat/falso'
...
const CREATE_QUERY = 'INSERT INTO shops (name, description, position, image, current_occupancy, total_occupancy) VALUES ($1, $2, point($3, $4), $5, $6, $7);';
const GET_QUERY = 'SELECT * FROM shops WHERE id=$1;';
const DELETE_QUERY = 'DELETE FROM shops WHERE id=$1;';
const UPDATE_QUERY = ({ name, description, position, image, current_occupancy, total_occupancy }: Nullable<ShopData>): { query: string, values: string[] } => {
    const query = [];
    const values = [];
    let paramIndex = 1;
    if (name !== null) {
...
function createShop(
    pool: Pool,
    { name, description, position: { latitude, longitude }, image, current_occupancy, total_occupancy }: ShopData,
): DatabaseReturn<ShopEntry> {
    return Result(async () => {
        return await pool.query<ShopEntry>(CREATE_QUERY, [
            name, description, latitude,
            longitude, image, current_occupancy, total_occupancy
        ]);
    });
}
