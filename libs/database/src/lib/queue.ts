...
const CREATE_QUERY = 'INSERT INTO shop_queue (shop_id, user_id, joined_at) VALUES ($1, $2, $3) ON CONFLICT (shop_id, user_id) DO NOTHING;'
const ENQUEUE_QUERY = 'INSERT INTO shop_queue (shop_id, user_id) VALUES ($1, $2) ON CONFLICT (shop_id, user_id) DO NOTHING;'
const DEQUEUE_QUERY = 'DELETE FROM shop_queue WHERE shop_id=$1 ORDER BY joined_at LIMIT 1;'
const DELETE_QUERY = 'DELETE FROM shop_queue WHERE shop_id=$1;';
const DELETE_ITEM_QUERY = 'DELETE FROM shop_queue WHERE shop_id=$1 AND user_id=$2;';
const GET_QUERY = 'SELECT * FROM shop_queue WHERE shop_id=$1 ORDER BY joined_at;'
...
function enqueue(
    ...
    return Result(async () => {
        return await pool.query<ShopQueueEntry>(ENQUEUE_QUERY, [
            shop_id,
            user_id
        ])
    })
...
function deleteItemQueue(
    ...
    return Result(async () => {
        return await pool.query<ShopQueueEntry>(DELETE_ITEM_QUERY, [shop_id, user_id]);
    });
}
